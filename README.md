...
- 合并转发现在对平台发送接口采用“至多一次”策略。
- `event.send()` 抛错时，平台可能已经收到消息；插件不会再盲目重试或追加 JSON，从而避免一次解析出现三份打包消息。
- 只有消息在调用发送接口前构建失败时，才会确定性地回退为一条完整 JSON。
- 旧版 `pack_retry_count` 和 `pack_retry_delay_ms` 配置继续保留以兼容现有配置，但不再触发不安全的发送重试。
