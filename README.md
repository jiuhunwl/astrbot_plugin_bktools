...
### `[Errno 20] Not a directory: ...`（例如 `.../metadata.yaml`、`.../_conf_schema.json`）

AstrBot 解压上传的 zip 时，会把压缩包 **`namelist()` 里的第一条路径** 当成插件根目录再去 `listdir`。若 zip **根目录是散文件**（没有把插件装进一层文件夹），第一条可能是 **`metadata.yaml`**、**`_conf_schema.json`** 等**文件**，就会报 `Not a directory`。

**推荐：** 在本仓库根目录执行 **`python package_plugin.py`**，使用生成的 **`astrbot_plugin_bktools.zip`** 上传。

**或手动正确打包：**

1. 在资源管理器中 **右键插件文件夹 `astrbot_plugin_bktools`**（选文件夹本身，不要进去全选里面的文件）→ **发送到 → 压缩(zipped)文件夹**，得到 zip 根下有一层 `astrbot_plugin_bktools/`，再上传。  
2. 使用 GitHub **Code → Download ZIP** 时，解压后先把 **`xxx-main` 文件夹改名为 `astrbot_plugin_bktools`**，再**右键该文件夹**压缩（不要只压解压出来的散文件）。也可 **git clone** 到 `plugins/astrbot_plugin_bktools`。

**错误示例：** 在仓库根目录里全选 `main.py`、`metadata.yaml` 等再压成 zip → 根目录没有统一文件夹，第一条常为 `metadata.yaml`，必触发本错误。

本仓库**已不在插件根目录附带** `.gitignore`，避免被打进包后成为「第一条路径」；自行打包时也不要把无关文件塞进 zip 根目录。

若已安装失败，请先删残留再重装：

- `data/plugins/plugin_upload_*`（上传失败产生的临时目录）  
- 不完整或损坏的 `data/plugins/astrbot_plugin_bktools`  

然后按上文 **正确结构** 重新上传，或改用 **git clone** 到 `plugins/astrbot_plugin_bktools`。
